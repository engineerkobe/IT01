...
import com.example.miku.test1.AppVariable.MyServerInfo;
import com.example.miku.test1.bean.Book;
import com.example.miku.test1.dao.BookDaoUtils;
...
//	public static String newsPath_url = "http://192.168.43.166:8080/DemoWeb/GetNewsServlet";
	public static String newsPath_url = "http://"+MyServerInfo.getUrl()+"/DemoWeb/GetNewsServlet";

	public static String BorrowingBooks_url = "http://"+MyServerInfo.getUrl()+"/DemoWeb/BorrowingBooks";
//	public static String BorrowingBooks_url = "http://192.168.43.166:8080/DemoWeb/BorrowingBooks";
	//封装新闻的假数据到list中返回
			...
			if(code == 200){
				...
				//2.解析获取的新闻数据到List集合中。
					...
					for (int i = 0 ;i < jsonArray.length();i++){//循环遍历jsonArray
						...
						books.setBOOKCASE(Json_book.getString("BOOKCASE"));
						arrayList.add(books);
	...
	public static Book BorrowingBooksForNetWork(String ISBN,String UID) {
		ArrayList<Book> arrayList = new ArrayList<Book>();
		try{
			//1.请求服务器获取新闻数据
			//获取一个url对象，通过url对象得到一个urlconnnection对象
			String tmp = BorrowingBooks_url + "?" +"ISBN="+ISBN+"&UID=" + UID;
			URL url = new URL(tmp);
			HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			//设置连接的方式和超时时间
			connection.setRequestMethod("GET");
			...
			if(code == 200){
				...
				if(!result.equals("NoBook")) {
					...
					JSONObject root_json = new JSONObject(result);//将一个字符串封装成一个json对象。
					//JSONArray jsonArray = root_json.getJSONArray("newss");//获取root_json中的newss作为jsonArray对象
					//JSONObject Json_book = jsonArray.getJSONObject(0);//获取一条新闻的json
					Book books = new Book();/*
					books.setISBN(Json_book.getString("ISBN"));
					books.setNAME(Json_book.getString("NAME"));
					books.setCOVER(Json_book.getString("COVER"));
					...
					books.setBOOKCASE(Json_book.getString("BOOKCASE"));
					*/
					books.setISBN(root_json.getString("ISBN"));
					books.setNAME(root_json.getString("NAME"));
					books.setCOVER(root_json.getString("COVER"));
					books.setBORROWER(root_json.getString("BORROWER"));
					books.setBOOKCASE(root_json.getString("BOOKCASE"));
					//3.清楚数据库中旧的数据，将新的数据缓存到数据库中
					//檢查是否有人借過書
					System.out.println(books.getBORROWER());
					return books;
				}else{return null;}
			}
